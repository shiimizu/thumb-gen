...
BGD_DECLARE(gdImagePtr) gdImageCreateFromPngCtx(gdIOCtx *infile) {
  ...
  case PNG_COLOR_TYPE_RGB_ALPHA:
    ...
    if (png_get_valid(png_ptr, info_ptr, PNG_INFO_tRNS)) {
      ...
      if (bit_depth == 16) { /* png_set_strip_16() not yet in effect */
        transparent =
            gdTrueColor(trans_color_rgb->red >> 8, trans_color_rgb->green >> 8,
...
BGD_DECLARE(void) gdImagePngEx(gdImagePtr im, FILE *outFile, int level, int basefilter) {
...
BGD_DECLARE(void *) gdImagePngPtrEx(gdImagePtr im, int *size, int level, int basefilter) {
  void *rv;
  gdIOCtx *out = gdNewDynamicCtx(2048, NULL);
  gdImagePngCtxEx(im, out, level, basefilter);
  rv = gdDPExtractData(out, size);
...
BGD_DECLARE(void) gdImagePngCtxEx(gdImagePtr im, gdIOCtx *outfile, int level,
  ...
  if (level != -1 && (level < 0 || level > 9)) {
    fprintf(stderr, "gd-png error: compression level must be 0 through 9");
    return;
  }
  ...
  if (!im->trueColor) {
    /* Oy veh. Remap the PNG palette to put the entries with interesting alpha
     * channel values first. This minimizes the size of the tRNS chunk and thus
     * the size of the PNG file as a whole.
     */
    ...
    if (tc) {
      ...
      /* (Semi-)transparent indexes come up from the bottom of the list of real
       * colors; opaque indexes come down from the top
       */
      j = 0;
      k = colors - 1;

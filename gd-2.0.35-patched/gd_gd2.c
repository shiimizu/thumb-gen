...
gdImageCreateFromGd2PartCtx(gdIOCtx *in, int srcx, int srcy, int w, int h) {
  ...
  int dstart, dpos;
  int i;
  /* 2.0.12: unsigned is correct; fixes problems with color munging. Thanks to
   * Steven Brown. */
  unsigned int ch;
  int vers, fmt;
  ...
  for (cy = scy; (cy <= ecy); cy++) {
    ...
    for (cx = scx; cx <= ecx; cx++) {
      ...
      if (!gd2_compressed(fmt)) {
        ...
        /* gd 2.0.11: gdSeek returns TRUE on success, not 0. Longstanding bug.
         * 01/16/03 */
        if (!gdSeek(in, dpos)) {
...
static void _gdImageGd2(gdImagePtr im, gdIOCtx *out, int cs, int fmt) {
   ...
   * seems a little silly to expect performance improvements on a 64x64 bit
   * scale, and 4096 because we buffer one chunk, and a 16MB buffer seems a
   * little large - it may be OK for one user, but for another to read it, they
   * require the buffer.
   */
  if (cs == 0) {
    cs = GD2_CHUNKSIZE;

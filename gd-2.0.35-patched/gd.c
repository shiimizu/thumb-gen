...
/* 2.0.10: cast instead of floor() yields 35% performance improvement. Thanks to
 * John Buckman. */
...
BGD_DECLARE(void) gdImagePaletteCopy(gdImagePtr to, gdImagePtr from) {
  ...
  for (y = 0; y < to->sy; y++) {
    for (x = 0; x < to->sx; x++) {
      ...
      if (xlate[p] == -1) {
        /* This ought to use HWB, but we don't have an alpha-aware version of
         * that yet. */
...
static int clip_1d(int *x0, int *y0, int *x1, int *y1, int maxdim) {
  ...
  if (*x0 >
      maxdim) { /* start of line is right of window - complement of above */
    if (*x1 > maxdim) { /* as is the end, so the line misses the window */
      return 0;
    }
    m = (*y1 - *y0) / (double)(*x1 - *x0); /* calculate the slope of the line */
    *y0 += (int)(m *
                 (maxdim - *x0)); /* adjust so point is on the right boundary */
    *x0 = maxdim;
...
static void gdImageBrushApply(gdImagePtr im, int x, int y) {
  ...
  if (im->trueColor) {
    if (im->brush->trueColor) {
      ...
      /* 2.0.12: Brush palette, image truecolor (thanks to Thorben Kundinger for
...
static void gdImageTileApply(gdImagePtr im, int x, int y) {
  ...
  if (im->trueColor) {
    ...
    if (p != gdImageGetTransparent(tile)) {
      if (!tile->trueColor) {
        p = gdTrueColorAlpha(tile->red[p], tile->green[p], tile->blue[p],
                             tile->alpha[p]);
      }
      gdImageSetPixel(im, x, y, p);
    }
...
static void gdImageAntiAliasedApply(gdImagePtr im, int px, int py) {
  ...
  if (((im->AAL_LAB_2 + LAC_2) >= LBC_2) &&
     ...
     */
    p_dist = fabs((float)((Ay_Cy * im->AAL_Bx_Ax) - (Ax_Cx * im->AAL_By_Ay)) /
                  im->AAL_LAB);
  } else {
  ...
  for (py = 0; py < im->sy; py++) {
    for (px = 0; px < im->sx; px++) {
      if (im->AA_opacity[py][px] != 0) {
        ...
        if ((old_color != color) && ((old_color != im->AA_dont_blend) ||
...
inline static void gdImageSetAAPixelColor(gdImagePtr im, int x, int y,
                                          int color, int t) {
...
BGD_DECLARE(void) gdImageCopy(gdImagePtr dst, gdImagePtr src, int dstX, int dstY, int srcX,
  ...
  for (y = srcY; y < (srcY + h); y++) {
    ...
    for (x = srcX; x < (srcX + w); x++) {
      ...
      if (src->trueColor) {
        /* 2.05: remap to the palette available in the destination image. This
         * is slow and works badly, but it beats crashing! Thanks to Padhrig
         * McCarthy.
         */
...
BGD_DECLARE(void) gdImageCopyMerge(gdImagePtr dst, gdImagePtr src, int dstX, int dstY, int srcX,
    ...
    for (x = srcX; (x < (srcX + w)); x++) {
      ...
      if (dst == src && pct == 100) {
        ...
        g = (0.29900f * gdImageRed(dst, dc)) +
            (0.58700f * gdImageGreen(dst, dc)) +
            (0.11400f * gdImageBlue(dst, dc));
...
BGD_DECLARE(void) gdImageCopyResized(gdImagePtr dst, gdImagePtr src, int dstX, int dstY, int srcX,
  ...
  /* Stretch vectors */
  int *stx, *sty;
  /* We only need to use floating point to determine the correct stretch vector
   * for one line's worth. */
  double accum;
  ...
  for (y = srcY; (y < (srcY + srcH)); y++) {
    for (ydest = 0; (ydest < sty[y - srcY]); ydest++) {
      ...
      for (x = srcX; (x < (srcX + srcW)); x++) {
        ...
        if (dst->trueColor) {
          /* 2.0.9: Thorben Kundinger: Maybe the source image is not a truecolor
           * image */
          if (!src->trueColor) {
          ...
          if (src->trueColor) {
            /* Remap to the palette available in the destination image. This is
             * slow and works badly. */
            mapTo = gdImageColorResolveAlpha(
                dst, gdTrueColorGetRed(c), gdTrueColorGetGreen(c),
                gdTrueColorGetBlue(c), gdTrueColorGetAlpha(c));
          } else {
...
BGD_DECLARE(void) gdImageCopyResampled(gdImagePtr dst, gdImagePtr src, int dstX, int dstY,
    ...
    for (x = dstX; (x < dstX + dstW); x++) {
      ...
      do {
        ...
        do {
          ...
          alpha_factor =
              ((gdAlphaMax - gdTrueColorGetAlpha(p))) * pcontribution;
...
BGD_DECLARE(void) gdImageOpenPolygon(gdImagePtr im, gdPointPtr p, int n, int c) {
  int i;
  int lx, ly;
  typedef void (*image_line)(gdImagePtr im, int x1, int y1, int x2, int y2,
                             int color);
...
BGD_DECLARE(void) gdImageFilledPolygon(gdImagePtr im, gdPointPtr p, int n, int c) {
  ...
  for (y = miny; y <= maxy; y++) {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (y >= y1 && y < y2) {
        im->polyInts[ints++] =
            (float)((y - y1) * (x2 - x1)) / (float)(y2 - y1) + 0.5 + x1;
...
int gdAlphaBlendOld(int dst, int src) {
   ...
   * pointing out the issue.
   */
  return ((((gdTrueColorGetAlpha(src) * gdTrueColorGetAlpha(dst)) / gdAlphaMax)
           << 24) +
          ((((gdAlphaTransparent - gdTrueColorGetAlpha(src)) *
             gdTrueColorGetRed(src) / gdAlphaMax) +
            (gdTrueColorGetAlpha(src) * gdTrueColorGetRed(dst)) / gdAlphaMax)
           << 16) +
          ((((gdAlphaTransparent - gdTrueColorGetAlpha(src)) *
             gdTrueColorGetGreen(src) / gdAlphaMax) +
            (gdTrueColorGetAlpha(src) * gdTrueColorGetGreen(dst)) / gdAlphaMax)
           << 8) +
          (((gdAlphaTransparent - gdTrueColorGetAlpha(src)) *
            gdTrueColorGetBlue(src) / gdAlphaMax) +
           (gdTrueColorGetAlpha(src) * gdTrueColorGetBlue(dst)) / gdAlphaMax));
}
...
static int gdLayerOverlay(int dst, int src) {
  ...
  return (((gdAlphaMax - a1 * a2 / gdAlphaMax) << 24) +
          ...
          (gdAlphaOverlayColor(gdTrueColorGetGreen(src),
           ...
           << 8) +
          (gdAlphaOverlayColor(gdTrueColorGetBlue(src), gdTrueColorGetBlue(dst),
                               gdBlueMax)));
}

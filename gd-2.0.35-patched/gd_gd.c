...
static gdImagePtr _gdCreateFromFile(gdIOCtx *in, int *sx, int *sy) {
  ...
  if (*sx == 65535 || *sx == 65534) {
    ...
    gd2xFlag = 1;
    /* 2.0.12: 65534 signals a truecolor .gd file. There is a slight redundancy
...
BGD_DECLARE(gdImagePtr) gdImageCreateFromGdCtx(gdIOCtxPtr in) {
  ...
  /* Then the data... */
  /* 2.0.12: support truecolor properly in .gd as well as in .gd2. Problem
   * reported by Andreas Pfaller. */
  if (im->trueColor) {
    for (y = 0; y < sy; y++) {
      for (x = 0; x < sx; x++) {
        int pix;
